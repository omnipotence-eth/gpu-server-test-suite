...
def _print_inventory(gpu_infos, system_info, pcie_infos):
    ...
    for gpu in gpu_infos:
        ...
        vram_str = f"{gpu.vram_total_mib} MiB ({gpu.vram_free_mib} MiB free)"
        gpu_table.add_row("VRAM", vram_str)
        gpu_table.add_row("ECC", gpu.ecc_mode)
        gpu_table.add_row("Temperature", f"{gpu.temperature_c} C")
        pwr_str = f"{gpu.power_draw_w:.1f} W / {gpu.power_limit_w:.1f} W limit"
...
def _build_test_registry(gpu_infos, config, profile):
    ...
    return {
        ...
        # Advanced Telemetry
        "xid_errors": lambda: run_xid_checks(gpu_infos, profile),
        "clock_throttle": lambda: run_clock_throttle_checks(gpu_infos, profile),
        "ecc_health": lambda: run_ecc_health_checks(gpu_infos, profile),
        # Interconnect & Topology
        "nvlink_p2p": lambda: run_nvlink_p2p(gpu_infos, profile),
        "nccl_validation": lambda: run_nccl_validation(gpu_infos, profile),
        "topology_map": lambda: run_topology_map(gpu_infos, profile),
        # Cleanup
...
def _output_results(
    ...
    if output_format == "junit":
        ...
        if junit_file:
            ...
            console.print(f"[green]JUnit XML written to: {junit_file}[/green]")
        else:
...
def diag(
    ...
    if metrics_port:
        ...
        console.print(f"[green]Prometheus metrics: http://localhost:{metrics_port}/metrics[/green]")
    ...
    # Build test registry
    test_registry = _build_test_registry(gpu_infos, config, profile)
...
def cleanup_cmd():
    ...
    console.print("[bold blue]Running GPU cleanup...[/bold blue]")
    ...
    for r in results:
        color = _status_color(r.status)
        console.print(f"  [{color}]{r.status.value}[/{color}] {r.message}")
...
def metrics(port):
    ...
    console.print(f"[bold blue]Starting Prometheus metrics server on port {port}...[/bold blue]")

    try:
        gpu_infos = get_all_gpus()
        get_metrics_store().update_gpu_metrics(gpu_infos)
    except Exception as e:
        console.print(f"[yellow]GPU detection failed: {e} — serving empty metrics[/yellow]")

    server = start_metrics_server(port=port, daemon=False)
    console.print(f"[green]Metrics available at: http://localhost:{port}/metrics[/green]")
...
def monitor(interval):
    ...
    console.print(
        f"[bold blue]GPU Health Monitor — refresh every {interval}s (Ctrl+C to stop)[/bold blue]"
    )
    try:
        with Live(refresh_per_second=1, screen=False) as live:
            while True:
                try:
                    ...
                    live.update(Panel(f"[red]GPU poll error: {e}[/red]", border_style="red"))
                time.sleep(interval)
    except KeyboardInterrupt:
...
def history(failures, limit):
    ...
    if not runs:
        if failures:
            ...
            console.print("[dim]No run history found. Run 'diag' to generate entries.[/dim]")
        return
